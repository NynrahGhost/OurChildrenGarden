...
public class UpdateTutor extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			params.add(request.getParameter("BirthDate"));
			
			
			DataBase.updateTutor(Integer.parseInt(request.getParameter("ParentID")), params, kidsAdd, kidsDelete, conn);
			conn2.createStatement().executeQuery("UPDATE Tutor SET Password = '"+request.getParameter("Password")+"' WHERE Phone = '"+request.getParameter("Tel")+"'");
